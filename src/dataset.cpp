...
Dataset::Dataset(const std::string &dataset_path, const PinholeStereoCamera &cam, int offset, int nmax, int step)
    ...
    boost::filesystem::path dataset_base(dataset_path);//得到boost格式的文件路径
    ...
    if (!boost::filesystem::exists(dataset_params))//数据集参数yaml文件
        throw std::runtime_error("[Dataset] Dataset parameters not found");
    YAML::Node dataset_config = YAML::LoadFile(dataset_params.string());//读取yaml文件的内容

    // setup image directories
    boost::filesystem::path img_l_dir = dataset_base / dataset_config["images_subfolder_l"].as<std::string>();
    boost::filesystem::path img_r_dir = dataset_base / dataset_config["images_subfolder_r"].as<std::string>();//images_subfolder_r: cam1/data/
...
bool Dataset::nextFrame(cv::Mat &img_l, cv::Mat &img_r) {
    ...
    // images_l和images_r都是双向链表 .front表示从链表前面开始读取图片
    img_l = cv::imread(images_l.front(), CV_LOAD_IMAGE_UNCHANGED);//UNCHANGED表示读取图片过程中不对图片进行任何处理,直接读取原始图片
    img_r = cv::imread(images_r.front(), CV_LOAD_IMAGE_UNCHANGED);
    cam.rectifyImagesLR(img_l, img_l, img_r, img_r);//图像去畸变
    images_l.pop_front();//读取完成之后弹出链表
    images_r.pop_front();

    return (!img_l.empty() && !img_r.empty());
}
// 两个链表中只要一个为空,则表示数据已经读取完毕
