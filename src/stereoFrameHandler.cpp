...
void StereoFrameHandler::initialize(const Mat img_l_, const Mat img_r_ , const int idx_)
{
    ...
    llength_th  = Config::minLineLength() * std::min( cam->getWidth(), cam->getHeight() ) ;//最短线段长度(单位:像素)
    // define StereoFrame
    prev_frame = new StereoFrame( img_l_, img_r_, idx_, cam );//新建一帧双目图像
    prev_frame->extractStereoFeatures( llength_th, orb_fast_th );//提取双目特征(点和线)
    prev_frame->Tfw     = Matrix4d::Identity();//当前帧和世界坐标系之间的变换矩阵
    prev_frame->Tfw_cov = Matrix6d::Identity();//变换矩阵的协方差
    prev_frame->DT      = Matrix4d::Identity();
    curr_frame = prev_frame;//当前帧=前一帧
    // SLAM variables for KF decision
    T_prevKF         = Matrix4d::Identity();//当前帧之前的关键帧的位姿(世界坐标系)
    cov_prevKF_currF = Matrix6d::Zero();//当前帧和之前关键帧的协方差
    prev_f_iskf      = true;//前一帧是关键帧(true)
    N_prevKF_currF   = 0;//当前帧和前一个关键帧的图像数目
}
...
void StereoFrameHandler::updateFrame()
{
    ...
    if( Config::adaptativeFAST() )
    {
        ...
        if( curr_frame->DT == Matrix4d::Identity() || curr_frame->err_norm > err_th ) //误差的norm大于阈值
...
void StereoFrameHandler::f2fTracking()
{
    ...
    // feature matching
    matched_pt.clear();//list< PointFeature* > matched_pt;//点特征
    ...
    n_inliers    = n_inliers_pt + n_inliers_ls;// 总内点线个数 = 内点个数+内线个数
}

void StereoFrameHandler::matchF2FPoints() // 当前帧和参考帧之间特征点匹配
{
    ...
    match(prev_frame->pdesc_l, curr_frame->pdesc_l, Config::minRatio12P(), matches_12);//min_ratio_12_l    = 0.9;        // parameter to avoid outliers in line matching
...
{
    SelfAdjointEigenSolver<Matrix6d> eigensolver(DTcov);//求协方差矩阵的特征值
    Vector6d DT_cov_eig = eigensolver.eigenvalues();//特征值从小到大排列

    if( DT_cov_eig(0)<0.0 || DT_cov_eig(5)>1.0 || err < 0.0 || err > 1.0 || !is_finite(DT) )//判断是否是好的值(特征值和err在0和1之间)
    {
...
void StereoFrameHandler::optimizePose()
{
    ...
    if( Config::useMotionModel() ) //利用匀速运动模型
    {
        // DT:变换矩阵  DT_cov:变换矩阵的协方差  e_prev:误差的模长
        DT     = prev_frame->DT;//紫色表示在该类定义的参数
        DT_cov = prev_frame->DT_cov;
        e_prev = prev_frame->err_norm;//误差的模长
        if( !isGoodSolution(DT,DT_cov,e_prev) ) //如果不是好的结果,则赋值为单位矩阵
    ...
    // optimization mode
    // 配置优化方法(GN, GNR, LM)
    int mode = 0;   // GN - GNR(Gn robust) - LM
    ...
    if( n_inliers >= Config::minFeatures() )
    {
        // optimize
        DT_ = DT;
        if( mode == 0 )      gaussNewtonOptimization(DT_,DT_cov,err,Config::maxIters());//计算结果
        ...
        {
            gaussNewtonOptimizationRobust(DT,DT_cov,err,Config::maxItersRef());// 高斯牛顿的鲁邦算法
            //DT     = Matrix4d::Identity();
            //cout << "[StVO] optimization didn't converge" << endl;
        }
    }
    else //如果内点太少,则变换矩阵为单位矩阵
    {
        DT     = Matrix4d::Identity();
...
void StereoFrameHandler::gaussNewtonOptimization(Matrix4d &DT, Matrix6d &DT_cov, double &err_, int max_iters)
{
    Matrix6d H;
    Vector6d g, DT_inc;
    double err, err_prev = 999999999.9;//很大的初值
    ...
    for( iters = 0; iters < max_iters; iters++)
    {
        ...
        if (err > err_prev) {
            if (iters > 0)
                break;
            err_ = -1.0;//首次迭代时误差比err_prev还大,则返回
            return;
        }
        ...
        DT_inc = solver.solve(g);//求解Hx=g
        DT  << DT * inverse_se3( expmap_se3(DT_inc) );//x_k+1 = x_k + delta_x
    ...
    DT_cov = H.inverse();  //DT_cov = Matrix6d::Identity();(协方差矩阵是H矩阵的逆)
    err_   = err;
}
...
void StereoFrameHandler::optimizeFunctions(Matrix4d DT, Matrix6d &H, Vector6d &g, double &e )
{
    ...
    H   = Matrix6d::Zero(); H_l = H; H_p = H;// 海塞矩阵 J^T*J
    g   = Vector6d::Zero(); g_l = g; g_p = g;//-J^T*e
    e   = 0.0;
    ...
    {
        if( (*it)->inlier )// KeyPoint*
        {
            Vector3d P_ = DT.block(0,0,3,3) * (*it)->P + DT.col(3).head(3);//Rp+t得到相机坐标系下的3D坐标
            Vector2d pl_proj = cam->projection( P_ );//得到像素坐标
            // projection error
            Vector2d err_i    = pl_proj - (*it)->pl_obs;//匹配的观测值(第二张图片上的点)
            double err_i_norm = err_i.norm();//误差的模长
            // estimate variables for J, H, and g
            double gx   = P_(0);//相机坐标系下的3D点
            double gy   = P_(1);
            double gz   = P_(2);
            double gz2  = gz*gz;
            double fgz2 = cam->getFx() / std::max(Config::homogTh(),gz2);
            double dx   = err_i(0);//误差项
            double dy   = err_i(1);
            // jacobian 雅克比矩阵
            Vector6d J_aux;
            J_aux << + fgz2 * dx * gz,
...
void StereoFrameHandler::removeOutliers(Matrix4d DT)
{
    ...
    if (n_inliers != (n_inliers_pt + n_inliers_ls))// 判断上述操作是否正确
