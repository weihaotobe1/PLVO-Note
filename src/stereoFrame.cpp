...
StereoFrame::StereoFrame(const Mat img_l_, const Mat img_r_ , const int idx_, PinholeStereoCamera *cam_) :
    ...
    inv_width  = GRID_COLS / static_cast<double>(img_l.cols);//逆宽度
...
StereoFrame::~StereoFrame()
{
    for( auto pt: stereo_pt ) //vector< PointFeature* > stereo_pt;//点特征
        delete pt;
...
void StereoFrame::extractStereoFeatures( double llength_th, int fast_th )
{
    ...
    if( Config::plInParallel() ) //点特征和线特征并行提取
    {
        // 模板函数 async 异步地运行函数 f （潜在地在可能是线程池一部分的分离线程中），并返回最终将保有该函数调用结果的
...
void StereoFrame::detectStereoPoints( int fast_th )
{
    ...
    if( Config::lrInParallel() ) //左右两幅图片并行提取特征
    {
        //vector<KeyPoint> points_l, points_r;//关键点
        // ref  是生成 std::reference_wrapper 类型对象的帮助函数，它们用模板实参推导确定结果的模板实参。
    ...
    {
        detectPointFeatures( img_l, points_l, pdesc_l, fast_th );//描述子是mat类型
...
void StereoFrame::matchStereoPoints( vector<KeyPoint> points_l, vector<KeyPoint> points_r, Mat &pdesc_l_, Mat pdesc_r, bool initial )
{
    ...
    // --------------------------------------------------------------------------------------------------------------------
    stereo_pt.clear();// vector<KeyPoint> stereo_pt
...
{
    bfm->knnMatch( pdesc_1, pdesc_2, pmatches_12, 2);//最近邻匹配
}
...
void StereoFrame::detectLineFeatures( Mat img, vector<KeyLine> &lines, Mat &ldesc, double min_line_length )
{
    ...
    if( Config::hasLines() )
    {
        ...
        if( !Config::useFLDLines() )
        {
            ...
            lsd->detect( img, lines, Config::lsdScale(), 1, opts);//lsd提取线段
            // filter lines
            ...
            lbd->compute( img, lines, ldesc);//lbd计算描述子
        }
        else
        {
...
{
    bfm->knnMatch( ldesc_1, ldesc_2, lmatches_12, 2);//线段特征的最近邻匹配
}
