...
Config::Config()
{
    ...
    max_kf_t_dist         = 5.0;//最大平移距离(t的模长)
    max_kf_r_dist         = 15.0;//最大旋转矩阵(旋转矩阵的模长)
    ...
    pl_in_parallel     = true;      // true if detecting points and line segments in parallel(并行检测点特征和线特征)
    best_lr_matches    = true;      // true if double-checking the matches between the two images(双重检测)
    ...
    // -----------------------------------------------------------------------------------------------------
    // Point features
    max_dist_epip     = 1.0;        // max. epipolar distance in pixels(最大极线距离)
    min_disp          = 1.0;        // min. disparity (avoid points in the infinite)(最小视差)
    ...
    max_iters_ref    = 10;          // max. number of iterations in the refinement stage(去除异常值之后重新进行优化)
...
// 函数定义为内联,编译器会复制函数到调用位置,保证速度
// 假如配置文件中定义了参数,则采用配置文件中的内容,否则返回默认参数(类构造的时候定义的参数)
template<typename T>
