...
public:
    ...
    /**
     * c++知识补充:
     *  类成员定义为静态的:无论有多少个类的对象,静态成员都只有一个副本
     *  在本类中表示所有从config文件中读取到的参数在程序运行中都是不变的
     */
    ...
    // 关键帧选取参数
    double min_entropy_ratio;// entropy:熵
    ...
    int    orb_nfeatures;  // orb特征参数
    double orb_scale_factor;
    int    orb_nlevels;
    int    orb_edge_th;
    int    orb_wta_k;
    int    orb_score;
    int    orb_patch_size;//图片分块
    int    orb_fast_th;
